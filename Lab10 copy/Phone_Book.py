...
def insert_manual():
    ...
    cur.execute("INSERT INTO localhost (first_name, last_name, phone_number) VALUES (%s, %s, %s)",
                (first, last, phone))
    conn.commit()
    print("Added")
...
def insert_from_csv(file_path):
    with open(file_path, newline='') as f:
        ...
        next(reader)
        for row in reader:
            cur.execute("INSERT INTO localhost (first_name, last_name, phone_number) VALUES (%s, %s, %s)", row)
    conn.commit()
    print("CSV added")
...
def search_user():
    ...
    query = "SELECT * FROM localhost WHERE 1=1"
    params = []
...
def delete_user():
    ...
    cur.execute("DELETE FROM localhost WHERE phone_number = %s", (phone,))
    conn.commit()
    print("Deleted")
